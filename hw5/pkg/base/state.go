...
func (s *State) HandleMessage(index int, deleteMessage bool) (result []*State) {
	result = []*State{}
	message := s.Network[index]
	targetNode := s.GetNode(message.To())
	newNodes := targetNode.MessageHandler(message)
	...
	for _, newNode := range newNodes {
		...
		newState.UpdateNode(message.To(), newNode)
...
func (s *State) TriggerNodeTimer(address Address, node Node) []*State {
	...
	for _, n := range node.TriggerTimer() {
...
func (s *State) RandomNextState() *State {
	...
	if roll < len(s.Network) {
		...
		message := s.Network[roll]

		switch {
		case s.isLocalCall(roll):
			return s.HandleMessage(roll, true)[0]
		case s.isDropOff:
			dropOffState := s.Inherit(DropOffEvent(message))
			dropOffState.DeleteMessage(roll)
			return dropOffState
		case s.isDuplicate:
